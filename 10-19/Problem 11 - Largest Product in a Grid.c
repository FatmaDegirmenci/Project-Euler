...
int main()
{
    ...
    long long int horizontal,vertical,diagonal,reverseDiagonal;
    int matrix[20][20] =     {
    ...
    for(i=3;i<17;i++)
    {
        for(j=0;j<17;j++)
        {
            ...
            reverseDiagonal = matrix[i][j]*matrix[i-1][j+1]*matrix[i-2][j+2]*matrix[i-3][j+3];
            ...
            if(reverseDiagonal>=maxProduct)
            {
                maxProduct = reverseDiagonal;
            }
        }
    }

    printf(" max product : %lld",maxProduct);
